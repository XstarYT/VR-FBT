...
Verification now includes synthetic multi-view camera-pose recovery and joint triangulation, 3D debug-scene rendering and interaction, one-to-three-source configuration, and enforcement of the three-phone cap, alongside the existing stable body calibration, filtering, MediaPipe, OSC, WebRTC/TLS, single-instance, and GUI coverage. Compilation, dependency checks, model presence, both phone-page JavaScript suites, and OpenSSL diagnostics remain part of the release check.
...
The local path discovers OpenSSL automatically, generates a private CA plus LAN-IP server certificate, and refreshes the server certificate when local addresses change. The CA remains local app data and is used only to sign the server certificate; it is not served to phones. Direct WebRTC has no STUN/TURN service and the WebSocket fallback also remains on LAN. The random link token, three-camera cap, latest-frame delivery and clean shutdown remain enforced.
